package by.mcs.graphql.dsl.jooq.relation


import by.mcs.graphql.dsl.jooq.type.JooqTableType
import by.mcs.graphql.fetcher.ProvidedDataFetcher
...
@PublicApi
class JooqOneToManyType<R : Record>(
    name: String?,
    description: String?,
  ...
  companion object {
    fun <R : Record> oneToMany(wrappedType: JooqTableType<R>, condition: Condition): Builder<R> {
      return Builder(wrappedType, condition)
    }

    fun <R : Record, T> oneToMany(wrappedType: JooqTableType<R>, foreignKey: TableField<*, T>): Builder<R> {
